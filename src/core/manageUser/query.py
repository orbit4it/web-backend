...
class Query:
    ...
    def get_users(self, info: Info) -> list[type.Users]:
        db = info.context['db']
        users = db.query(model.User).filter(model.User.role != 'superadmin' or model.User.role != 'admin').all()
    ...
    def get_users_divisions(self, info: Info, division_id: int)->list[type.Users]:
        db = info.context['db']
        users = db.query(model.User).filter(model.User.division_id == division_id and model.User.role != 'superadmin').all()
        return [extract_user_data(user) for user in users]
